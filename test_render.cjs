...
CONFIG.lectures.forEach((lec, i) => {
    if (typeof lec.title !== 'string') { console.log(`Error in title index ${i}:`, lec.title); hasError = true; }
    if (typeof lec.description !== 'string') { console.log(`Error in description index ${i}:`, lec.description); hasError = true; }
});
